import { NextResponse } from 'next/server';
import { z } from 'zod';
import { getCurrentSession } from '@/lib/auth';
import { scheduleManager } from '@/lib/engines/Schedule_Manager';
import { MemoryStoreError } from '@/lib/memory/MemoryStore';
import { enforceRateLimit, recordLiveEvent } from '@/lib/postgres';
import { assertTrustedOrigin, CorsValidationError } from '@/lib/security';

export const dynamic = 'force-dynamic';

const studySchema = z.object({
  subject: z.string().trim().min(1).max(120),
  topic: z.string().trim().max(120).optional(),
  durationMinutes: z.number().int().min(5).max(720),
  confidenceScore: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5)]).optional(),
});

export async function POST(request: Request) {
  try {
    assertTrustedOrigin(request);

    const session = await getCurrentSession();

    if (!session) {
      return NextResponse.json({ error: 'You must be signed in.' }, { status: 401 });
    }

    await enforceRateLimit({
      scope: 'study-log',
      key: session.user.id,
      maxAttempts: 30,
      windowMs: 5 * 60 * 1000,
    });

    const payload = studySchema.parse(await request.json());
    const studySession = await scheduleManager.logStudySession({
      studentId: session.user.email,
      subject: payload.subject,
      topic: payload.topic,
      durationMinutes: payload.durationMinutes,
      confidenceScore: payload.confidenceScore,
    });

    await recordLiveEvent('study_logged', {
      label: `${studySession.subject} / ${studySession.topic}`,
      userId: session.user.id,
      studentId: session.user.email,
    });

    return NextResponse.json({
      success: true,
      message: `Logged ${studySession.durationMinutes} minutes for ${studySession.subject}.`,
      studySession,
    });
  } catch (error) {
    console.error('Study session route failed', error);

    if (error instanceof z.ZodError) {
      return NextResponse.json(
        {
          error: 'Provide a valid subject, duration, and optional topic/confidence.',
        },
        { status: 400 },
      );
    }

    if (error instanceof Error && error.message === 'Too many requests. Please try again later.') {
      return NextResponse.json({ error: error.message }, { status: 429 });
    }

    if (error instanceof CorsValidationError) {
      return NextResponse.json({ error: 'Forbidden request origin.' }, { status: 403 });
    }

    if (error instanceof MemoryStoreError) {
      return NextResponse.json(
        {
          error: `Hindsight memory is currently unavailable. ${error.message}`,
        },
        { status: 503 },
      );
    }

    return NextResponse.json(
      {
        error: 'Unable to log the study session.',
      },
      { status: 500 },
    );
  }
}
