...
interface StudyCompanionShellProps {
  studentId: string;
  viewerName?: string;
  showLogout?: boolean;
  children: ReactNode;
...
export default function StudyCompanionShell({
  studentId,
  viewerName,
  showLogout = false,
  children,
}: StudyCompanionShellProps) {
  ...
  return (
    <main className="relative min-h-screen text-[var(--color-text)]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1500px] flex-col gap-5 px-4 py-6 sm:px-6 lg:grid lg:grid-cols-[320px_minmax(0,1fr)] lg:gap-6 lg:px-8">
        <aside className="app-shell-panel flex flex-col gap-6 p-5 lg:sticky lg:top-6 lg:h-[calc(100vh-3rem)] lg:overflow-y-auto">
